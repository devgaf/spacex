...
public class SpexController {
     ...
     * utilizarlo al hacer las peticiones HTTP a la API de SpaceX.
     */
    ...
    /**
     * Maneja excepciones generales no capturadas por métodos más específicos.
     *
     * @param e La excepción lanzada que no fue manejada por otros métodos.
    ...
    /**
     * <p>Obtiene el listado de lanzamientos de SpaceX en formato JSON.</p>
     * 
     * Si falla al obtener el contenido, lanza una NoContentException con el http status 204 (NO_CONTENT).
     * Si falla al configurar SSL, lanza una SSLConfigurationException con el http status 500 (INTERNAL_SERVER_ERROR).
     * Si falla al obtener el contenido, lanza una IOException con el http status 500 (INTERNAL_SERVER_ERROR).
     * Si falla por cualquier otro motivo, lanza una Exception con el http status 500 (INTERNAL_SERVER_ERROR).
     *
